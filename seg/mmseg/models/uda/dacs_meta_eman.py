...
@UDA.register_module()
class DACS_META_EMAN(UDADecorator):

    def __init__(self, **cfg):
        super(DACS_META_EMAN, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def forward_train(
        ...
        # ema_forward_logit = ema_forward_result['logits']
        ...
        # Lấy thông số của img data từ "img_norm_cfg"
        ...
        if not self.source_only:
            ...
            # MY INFO fused:  torch.Size([2, 512])
            # print("MY INFO fused: ", fused.shape)

            # fused = torch.mean(fused)
            # print("MY INFO fused: ", fused)
            # MY INFO fused:  tensor(0., device='cuda:0')
            ...
            # print(ema_forward_log_vars)
            # OrderedDict([('decode.loss_seg', 2.874251365661621), ('decode.acc_seg', 9.85565185546875), ('loss', 2.874251365661621)])
        ...
        # Clean loss? Supervised loss hay adaptation loss?
        clean_loss, clean_log_vars = self._parse_losses(clean_losses)

        log_vars.update(clean_log_vars)

        # Sao lại backward rồi???
